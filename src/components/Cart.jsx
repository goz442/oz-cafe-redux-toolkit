        ...
        {cart.length ? (
          cart.map((el, idx) => (
            <CartItem
              ...
              removeItem={() => dispatch(removeFromCart(el.id))}
            />
          ))
        ) : (
...
function CartItem({ item, options, quantity, removeItem }) {
  return (
    <li className="cart-item">
      ...
      <button className="cart-item-delete" onClick={removeItem}>삭제</button>
    </li>
  );
}
